...
<%
		...
		sql = "select Start_year from movie where id =" + movieID;
		...
		if(aaa != null)
		{
			...
			try{
				...
				{
					sql = "update movie set End_year = TO_DATE('"+endYear+"', 'yyyy-mm-dd') where id = "+movieID;
					...
					if(res != 0)
					{
						...
						out.println("<script>alert('end Year Update success!');");
						out.println("location.href  ='modifyMovie.jsp';</script>");

					}
				}
				else
				{
					out.println("<script>alert ('Must be before start year');");
		...
		else
		{
			...
			catch(Exception e)
			{
				...
				out.println("location.href  ='ModifyMovieInfo.jsp';</script>");
			}
		}
